...
main(int argc, char **argv)
{
    ...
    MPI_Bcast(X, BLOCK_ROW_SIZE*BUFFER_SIZE,MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    for (int task = 0; task < ntasks; task++) {
        ...
        //double **X = hyperslabread("x.h5", "/X", BLOCK_ROW_SIZE, BUFFER_SIZE, x_offset, 0);
        //MPI_Bcast(X, BLOCK_ROW_SIZE*BUFFER_SIZE,MPI_DOUBLE, 0, MPI_COMM_WORLD);
        MPI_Scatter (X, BLOCK_ROW_SIZE*BUFFER_SIZE, MPI_DOUBLE, 
                     X, BLOCK_ROW_SIZE*BUFFER_SIZE, MPI_DOUBLE,
